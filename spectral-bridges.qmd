...
Using the symmetrical nature of the numerator, the calculation of the non-transformed affinity matrix can be subdivided into five steps highlighted by @alg-affinity-construction. After computing this first affinity matrix, the non-linear transformation $g$ can then be applied. Note all operations have to be understood element-wise except for a dot product. Compared to a naive implementation in Python, the following method is approximately 200 times faster on a small dataset comprised of $n = 3594$, $d = 2$ points, and a value of $m = 250$.
...
\Comment{$\text{voronoiRegions}$: array of $m$ Voronoi regions, $\text{centroids}$: the centers of each region}
    ...
    \Comment{Symmetrize and normalize the affinity matrix}
