...
Taking a closer look at the second step of Algorithm @alg-spectral-bridges for affinity matrix calculation with a $O(m^2d)$ time complexity, most operations can be parallelized leaving a single loop, thus allowing for an efficient matrix construction. 
...
\begin{algorithmic}
\Procedure{Affinity}{$\text{voronoiRegions}, \text{centroids}$}
