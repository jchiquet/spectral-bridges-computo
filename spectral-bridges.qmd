...
\Comment{$\text{voronoiRegions}$: array of $m$ Voronoi regions, $\text{centroids}$: the centers of each Voronoi region}
    ...
    \Comment{Calculate the segments tensor}
    \State $\text{norms} \gets \{(\|\text{segments}[k, l]\|^4)\}_{kl}$
    \Comment{Compute the fourth power of the norms of the segments}
    ...
    \For{$i \in \{1, ..., m\}$}
        ...
        \Comment{Element-wise maximum following a dot product yielding a $n_i \times m$ matrix}
