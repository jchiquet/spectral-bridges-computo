...
\Comment{$X$: input dataset, $k$: number of clusters, $m$: number of Voronoi regions}
...
Using the symmetrical nature of the numerator, the calculation of the non-transformed affinity matrix can be subdivided into five steps highlighted by @alg-affinity-construction. After computing this first affinity matrix, the non-linear transformation $g$ can then be applied. Compared to a naive implementation, the following method is approximately 200 times faster on a small dataset comprised of $n = 3594$, $d = 2$ points, and a value of $m = 250$.
...
\begin{algorithmic}
\Procedure{AffinityConstruction}{$\text{voronoiRegions}, \text{centroids}$}
\Comment{$\text{voronoiRegions}$: array of $m$ Voronoi regions, $\text{centroids}$: the centers of each Voronoi region}

    \State \textbf{Step 1: Data Centering}
    \For{$i \in \{1, ..., m\}$}
        \State $\text{centeredVoronoiRegions}[i] \gets \text{voronoiRegions}[i] - \text{centroids}[i]$
        \Comment{Center each Voronoi region by its center}
    \EndFor

    \State \textbf{Step 2: Cardinal Calculation}
    \State $\text{counts} \gets \{(n_k + n_l)\}_{kl}$
    \Comment{Calculate the number of combined points in each pair of Voronoi regions}

    \State \textbf{Step 3: Segments Normalization Calculation}
    \State $\text{segments} \gets \{(\text{centroids}[l] - \text{centroids}[k])\}_{kl}$
    \Comment{Calculate the $m \times m \times d$ segments tensor}
    \State $\text{norms} \gets \{(\|\text{segments}[k, l]\|^4)\}_{kl}$

    \State \textbf{Step 4: Projection Calculation}
    \Comment{Calculate half of the numerator}
    \For{$i \in \{1, ..., m\}$}
        \State $\text{projections} \gets \max\left(0, \operatorname{dot}(\text{centeredVoronoiRegions}[i], \text{segments}[i].T)\right)$
        \Comment{Element-wise maximum following a dot product between the centered Voronoi region data matrix and the $i$-th row of the semgents tensor, yielding a $n_i \times m$ matrix}
        \State $\text{affinity}[i] \gets \{(\sum_k \text{projections}[k, j]^2)\}_j$
        \Comment{Assign each row of the affinity matrix}
    \EndFor

    \State \textbf{Step 5: Final Affinity Matrix Calculation}
    \State $\text{affinity} \gets (\text{affinity} + \text{affinity}.T) / (\text{counts} * \text{norms})$
    \Comment{Symmetrize and normalize the affinity matrix with element-wise multiplication and division}

    \State \Return $g(\text{affinity})$
    \Comment{Return the affinity matrix}
\EndProcedure
\end{algorithmic}
\end{algorithm}
```
