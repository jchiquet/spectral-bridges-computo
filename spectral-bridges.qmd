...
Other approaches use the properties of the small initial cluster for the affinity computation. Clustering Based on Graph of Intensity Topology (GIT) estimates a global topological graph (topo-graph) between local clusters [@gao2021git]. It then uses the Wasserstein Distance between predicted and prior class proportions to automatically cut noisy edges in the topo-graph and merge connected local clusters into final clusters. 
...
Considering two centroïds, the normalized average of the difference betweenn Bridge and balls inertia  (See [Appendix](#gain)) constitutes the basis of our affinity measure between to regions:
$$
\begin{aligned}
...
\Procedure{SpectralBridges}{$X, k, m$} \Comment{$X$: input dataset, $k$: number of clusters, $m$: number of Voronoi regions}
    ...
    \State $\text{centroids}, \text{voronoiRegions} \gets$ \Call{KMeans}{$X, m$} \Comment{Initial centroids and Voronoi Regions  using k-means++}
      ...
      \State \textbf{Step 4: Propagate} \Comment{Affect each data point to the cluster of its region}
      \State $\text{clusters} \gets$ \Call{Propagate}{$X, \text{labels}, \text{voronoiRegions}$}
     \State \Return $\text{clusters}$ \Comment{Cluster labels for data points in $X$}
\EndProcedure
\end{algorithmic}
\end{algorithm}
```
...
Taking a closer look at the second step routine for affinity matrix calculation with a $O(m^2d)$ time complexity, most operations can be parallelized leaving a single loop, thus allowing for an efficient matrix construction. 
Recall: 

$$
a_{kl} = \frac{\sum_{\boldsymbol{x_i} \in \mathcal V_k} \langle \boldsymbol{x_i} - \boldsymbol{\mu}_k \vert \boldsymbol{\mu}_l - \boldsymbol{\mu}_k \rangle_+^2  \sum_{\boldsymbol{x_i} \in \mathcal V_l} \langle \boldsymbol{x_i} - \boldsymbol{\mu}_l \vert \boldsymbol{\mu}_k - \boldsymbol{\mu}_l\rangle_+^2}{(n_k+n_l)\|\boldsymbol \mu_k - \boldsymbol \mu_l\|^4}
$$

Using the symmetrical nature of the numerator, the calculation of the affinity matrix can be subdivided into five steps highlighted by @alg-affinity-construction. Compared to a naive implementation, the following method is approximately 200 times faster on a small dataset comprised of $n = 3594$, $d = 2$ points, and a value of $m = 250$.

```pseudocode
#| label: alg-affinity-construction
#| html-indent-size: "1.2em"
#| html-comment-delimiter: "//"
#| html-line-number: true
#| html-line-number-punc: ":"
...
\begin{algorithm}
\caption{Affinity matrix construction}
\begin{algorithmic}
\Procedure{AffinityConstruction}{$\text{voronoiRegions}, \text{centroids}$} \Comment{$\text{voronoiRegions}$: list of $m$ Voronoi regions, $\text{centroids}$: the centers of each Voronoi region}
    \State \textbf{Step 1: Data Centering}
    \For{$i \in \{1, ..., m\}$}
        \State $\text{centeredVoronoiRegions}[i] \gets \text{voronoiRegions}[i] - \text{centroids}[i]$ \Comment{Centering each Voronoi region by their center}
    \EndFor
    \State \textbf{Step 2: Cardinal Calculation}
    \State $\text{counts} \gets \{(n_k + n_l)\}_{kl}$ \Comment{Calculate the number of combined points in each pair of Voronoi regions}
    \State \textbf{Step 3: Segments Normalization Calculation}
    \State $\text{segments} \gets \{(\text{centroids}[l] - \text{centroids}[k])\}_{kl}$ \Comment{Calculate the segments tensor}
    \State $\text{norms} \gets \{(\|\text{segments}[k, l]\|^4)\}_{kl}$
    \State \textbf{Step 4: Projection Calculation} \Comment{Calculate half of the numerator}
    \For{$i \in \{1, ..., m\}$}
        \State $\text{projections} \gets \max\left(0, \operatorname{dot}(\text{centeredVoronoiRegions}[i], \text{segments}[i].T)\right)$ \Comment{Elementwise maximum following a dot product yielding a $n_i \times m$ matrix}
        \State $\text{affinity}[i] \gets \sum_j \operatorname{power}(\text{projections}[j], 2)$ \Comment{Assign each row of the matrix, summing elementwise squared projections along rows of each $\text{projections}[i]$ matrix}
    \EndFor
    \State \textbf{Step 5: Final Affinity Matrix Calculation}
    \State $\text{affinity} \gets (\text{affinity} + \text{affinity}.T) / (\text{counts} * \text{norms})$ \Comment{Symmetrize and normalize the affinity matrix with elementwise multiplication and division}
    \State \Return $\text{affinity}$ \Comment{Return the affinity matrix}
\EndProcedure
\end{algorithmic}
\end{algorithm}
```
...
<!--We evaluate the algorithms on both raw and PCA-processed data with varying dimensionality. For synthetic datasets, we introduce Gaussian and/or uniform noise to evaluate the robustness of our algorithm.
-->
...
<!-- We set the hyperparameters of our algorithm to "reasonable" values based on the size of each dataset and the number of clusters. A larger number of clusters typically suggests that a higher value for the Voronoi regions is optimal. Conversely, using a high number of Voronoi regions for a small dataset might result in nearly empty regions that do not adequately represent any local structure. We will discuss our method's sensitivity to this parameter, but at no point did we use labels to determine its empirically optimal value.
-->

We set the hyperparameters of our algorithm based on the size of each dataset, $n$, and the number of clusters, $K$.
A larger number of clusters typically suggests that a higher value for the number of Voronoi regions is optimal. Conversely, using a high number of Voronoi regions for a small dataset might result in nearly empty regions that do not adequately represent any local structure.

A good yet not very precise way of setting the number of Voronoi regions $m$ is to observe the Within Cluster Sum of Squares (WCSS) or inertia in a way akin to the elbow method. Since $m$ should be set to a value strictly greater than $K$, we plot the WCSS for varying values of $m$, and find a value such that the WCSS-$m$ relationship becomes quasi-linear.

By adjusting $m$ in this manner, we aim to balance the need for detailed representation with the risk of overfitting, ensuring that each Voronoi region meaningfully captures the underlying data distribution.
...
We first evaluated our algorithm's accuracy on the MNIST dataset. Metrics were collected to compare our method with k-means++, EM, and Ward clustering. Metric were estimated by taking the empirical average over $10$ consecutive runs with the same random seed for each method. Since our computational capabilites were too limited, we sampled 20,000 (one third) data points at random. This sample is not fixed and changes for each iteration.

Let $h$ denote the embedding dimension of the dataset. We tested our method on the raw MNIST dataset without preprocessing ($h = 784$) and after reducing its dimension using PCA to $h \in \{8, 16, 32, 64\}$ (see @fig-mnist-scores).
...
We also put our algorithm to the test against the same competitors using scikit-learn's UCI Breast Cancer data. Once again, our method performs well although the advantage is not as obvious in this case (see @fig-cancer-scores). However, in none of our tests has it ranked worse than k-means++. The results are displayed as a boxplot generated from $200$ iterations of each algorithm using a different seed, in order to better grasp the variability lying in the seed dependent nature of the k-means++, Expectation Maximization and Spectral Bridges algorithms.
...
<!--Since we expect our algorithm to shine at discerning complex and intricate cluster structures, we collected an array four toy datasets illustrated in Figure @fig-toy-datasets. 
-->
